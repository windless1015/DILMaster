...
public:
  ...
  void setup(IBMSolver &ibm, DEMSolver &dem, StepContext &ctx) {
    ...
    // Allocate & Init
    // Allocate & Init (Fix: Init first to load STL/count, then Allocate fields,
    // then Init again to upload)
    ibm.initialize(ctx);
    ibm.allocate(ctx);
    ibm.initialize(ctx);
    ...
    // depending on blade geometry. Propeller radius ~0.2? (Assuming standard
    // asset)
...
private:
  ...
  void setupIBM(IBMSolver &ibm) {
    std::string full_path =
        cfg_.stl_file; // Assume relative path from working directory
    ...
    iCfg.rotation_center_x = 0.5f;
    iCfg.rotation_center_y = 0.5f;
    iCfg.rotation_center_z = 0.5f;
  ...
  void setupDEM(DEMSolver &dem) {
    ...
    dCfg.kn =
        cfg_.stiffness; // Use same stiffness if possible, or internal material
    dCfg.restitution = 1.0f; // Elastic

    dCfg.domain_min_x = 0;
    dCfg.domain_max_x = 1.0f;
    dCfg.domain_min_y = 0;
    dCfg.domain_max_y = 1.0f;
    dCfg.domain_min_z = 0;
    dCfg.domain_max_z = 1.0f;
