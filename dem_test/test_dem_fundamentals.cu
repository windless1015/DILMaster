...
bool Test_T0_ElasticBounce() {
    ...
    std::vector<float3> pos(1, make_float3(0.5f, 0.5f, 5.0f));
    std::vector<float3> vel(1, make_float3(0.0f, 0.0f, 0.0f));
    std::vector<float> rad(1, 0.1f);
    
    std::vector<float> pos_soa(3);
    std::vector<float> vel_soa(3);
    core::ArrayLayoutConverter::AoSToSoA_float3(pos, pos_soa.data());
    core::ArrayLayoutConverter::AoSToSoA_float3(vel, vel_soa.data());

    core->uploadPositions(pos_soa.data());
    core->uploadVelocities(vel_soa.data());
    ...
    for (int i = 0; i < 2000; ++i) { // 2 seconds
        ...
        core->downloadPositions(pos_soa.data());
        core->downloadVelocities(vel_soa.data());
        pos = core::ArrayLayoutConverter::SoAToAoS_float3(pos_soa.data(), 1);
        vel = core::ArrayLayoutConverter::SoAToAoS_float3(vel_soa.data(), 1);
...
bool Test_T2_Momentum() {
    ...
    std::vector<float3> pos(2);
    pos[0] = make_float3(-0.6f, 0.0f, 0.0f);
    pos[1] = make_float3(0.6f, 0.0f, 0.0f);
    
    std::vector<float3> vel(2);
    vel[0] = make_float3(1.0f, 0.0f, 0.0f);
    vel[1] = make_float3(-1.0f, 0.0f, 0.0f);
    
    std::vector<float> rad = { 0.1f, 0.1f };

    std::vector<float> pos_soa(3 * 2);
    std::vector<float> vel_soa(3 * 2);
    core::ArrayLayoutConverter::AoSToSoA_float3(pos, pos_soa.data());
    core::ArrayLayoutConverter::AoSToSoA_float3(vel, vel_soa.data());
    ...
    core->downloadVelocities(vel_soa.data());
    vel = core::ArrayLayoutConverter::SoAToAoS_float3(vel_soa.data(), 2);
     printf("    [T2] V_left=%.4f, V_right=%.4f\n", vel[0].x, vel[1].x);
    
    // Should rebound
    ASSERT_LT(vel[0].x, -0.9f); // Left particle moving left
    ASSERT_GT(vel[1].x, 0.9f);  // Right particle moving right

    float p_final = vel[0].x + vel[1].x;
...
bool Test_T3_InclineFriction() {
    ...
    // Case A: Stick
    {
        ...
        std::vector<float3> pos = {make_float3(0.5f, 0.5f, cfg.particle_radius - 1e-4f)};
        std::vector<float3> vel = {make_float3(0.0f, 0.0f, 0.0f)};
        ...
        ASSERT_NEAR(vel[0].x, 0.0f, 0.05f);
    }
    // Case B: Slide
    {
...
bool Test_T4_Packing() {
    ...
    auto core = createCore(cfg);
    std::vector<float3> pos(N), vel(N, make_float3(0.0f,0.0f,0.0f));
    std::vector<float> rad(N, 0.05f);
    int idx = 0;
    for(int z=0; z<N_side; ++z) {
        for(int y=0; y<N_side; ++y) {
            for(int x=0; x<N_side; ++x) {
                pos[idx] = make_float3(0.2f + x * 0.12f,
                                       0.2f + y * 0.12f,
                                       0.5f + z * 0.12f);
                idx++;
            }
    ...
    std::vector<float> pos_soa(3*N), vel_soa(3*N);
    core::ArrayLayoutConverter::AoSToSoA_float3(pos, pos_soa.data());
    core::ArrayLayoutConverter::AoSToSoA_float3(vel, vel_soa.data());
    ...
    core->downloadVelocities(vel_soa.data());
    vel = core::ArrayLayoutConverter::SoAToAoS_float3(vel_soa.data(), N);
    float total_ke = 0.0f;
    for(auto v : vel) total_ke += v.x*v.x + v.y*v.y + v.z*v.z;
...
bool Test_T5_Convergence() {
    auto run_sim = [&](float dt) -> float {
        ...
        auto core = createCore(cfg);
        std::vector<float3> pos(1, make_float3(0.5f, 0.5f, 1.0f)); // Shorter drop for faster convergence test
        std::vector<float3> vel(1, make_float3(0.0f, 0.0f, 0.0f));
        std::vector<float> rad(1, 0.1f);
        ...
        for(int i=0; i<steps; ++i) {
            ...
            pos = core::ArrayLayoutConverter::SoAToAoS_float3(pos_soa.data(), 1);
            vel = core::ArrayLayoutConverter::SoAToAoS_float3(vel_soa.data(), 1);
            if(vel[0].z > 0.5f && pos[0].z > 0.11f) bounced = true;
            if(bounced && pos[0].z > max_z) max_z = pos[0].z;
        }
        return max_z;
    };
