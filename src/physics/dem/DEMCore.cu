...
void DEMCore::configure(const DEMConfig &cfg) {
    ...
    // Auto-compute damping from restitution
    ...
    // Compute separate gammas for Textbook Accuracy
    gamma_n_pp_ = computeDampingFromRestitution(cfg_.material.restitution, cfg_.material.kn, m_eff_pp);
    gamma_n_pw_ = computeDampingFromRestitution(cfg_.material.restitution, cfg_.material.kn, m_eff_pw);

    // Default tangential damping to same as normal if not specified
    gamma_t_pp_ = gamma_n_pp_;
    gamma_t_pw_ = gamma_n_pw_;
    
    // Critical time step
    float t_osc = 3.14159f * std::sqrt(m_eff_pp / cfg_.material.kn);
    t_critical_ = t_osc;
    mass_min_ = m_default;
    
    printf("[DEMCore Config] e=%.3f, kn=%.1e\n", cfg_.material.restitution, cfg_.material.kn);
    printf("                 m_pp=%.2e, gamma_pp=%.2e, t_c=%.2e\n", m_eff_pp, gamma_n_pp_, t_critical_);
}
...
void DEMCore::computeParticleContacts(float dt) {
    ...
    if (cfg_.use_naive_n2) {
        ...
        kComputeCollisionsNaive<<<blocks, 128>>>(
            ...
            cfg_.material.kn, gamma_n_pp_, cfg_.material.mu,
            cfg_.material.kt, gamma_t_pp_, dt, N);
        CUDA_CHECK(cudaGetLastError());
        return;
    }
    ...
    kComputeParticleContacts<<<blocks, 128>>>(
        ...
        gamma_n_pp_, gamma_t_pp_, cfg_.material.mu,
        dt,
...
void DEMCore::computeWallContacts(float dt) {
    ...
    kComputeWallContacts<<<blocks, 256>>>(
        ...
        d_contacts_old_, d_contacts_new_, contact_mask_,
        cfg_.material.kn, cfg_.material.kt,
        gamma_n_pw_, gamma_t_pw_, cfg_.material.mu,
        dt,
