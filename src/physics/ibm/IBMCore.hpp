...
public:
   ...
   * @param dt 时间步长
   * @param valid_mask 有效流体单元掩码 (Host/Device? Device pointer. 1=Valid,
   * 0=Invalid/Gas) [Optional]
...
private:
  ...
  unsigned int *d_health_flags_ =
      nullptr; // 健康状态标志 (0=OK, 1=NaN, 2=Explosion)
...
public:
  ...
  // 访问接口
  float3 *getMarkerPositionDevice() {
    return backend_.getMarkerPositionDevice();
  }
  const float3 *getMarkerPositionDevice() const {
    return backend_.getMarkerPositionDevice();
  }
  float3 *getMarkerVelocityDevice() {
    return backend_.getMarkerVelocityDevice();
  }
  const float3 *getMarkerVelocityDevice() const {
    return backend_.getMarkerVelocityDevice();
  }
  float *getMarkerAreaDevice() { return backend_.getMarkerAreaDevice(); }
  const float *getMarkerAreaDevice() const {
    return backend_.getMarkerAreaDevice();
  }
